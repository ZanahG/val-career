    ...
    <main className="vct-screen">
      ...
      <div className="vct-career-layout">
        ...
        <section className="vct-content">
          ...
          {!narrativeEvent && lastMatch && (
            <section className="vct-last-match">
              <span className={lastMatch.won ? "result-badge result-badge--win" : "result-badge result-badge--loss"}>{lastMatch.won ? "W" : "L"}</span>
              <div><strong>vs {getTeamById(lastMatch.opponentId)?.name}</strong><small>{lastMatch.scoreFor}-{lastMatch.scoreAgainst}</small></div>
              <div><span>RATING</span><strong>{lastMatch.playerRating.toFixed(2)}</strong></div>
