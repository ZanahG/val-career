...
const PLAYER_VISUAL_WIDTH = 7;
const PLAYER_VISUAL_HEIGHT = 9;

const PLAYER_HITBOX_WIDTH = 3.8;
const PLAYER_HITBOX_HEIGHT = 5.5;

const PLAYER_HITBOX_OFFSET_X = 1.6;
const PLAYER_HITBOX_OFFSET_Y = 2.3;

const ENEMY_HITBOX_HEIGHT = 2.2;
  ...
  useEffect(() => {
    ...
    const frame = (time:number) => {
      ...
      if (phaseRef.current === "waiting") {
        ...
        if (nextX + PLAYER_VISUAL_WIDTH >= 98) {
          nextX = 98 - PLAYER_VISUAL_WIDTH;
          walkDirectionRef.current = -1;
        }
      ...
      if (phaseRef.current === "committed") {
        ...
        const playerLeft = playerXRef.current + PLAYER_HITBOX_OFFSET_X;
        const playerRight = playerLeft + PLAYER_HITBOX_WIDTH;

        const playerTop = nextY + PLAYER_HITBOX_OFFSET_Y;
        const playerBottom = playerTop + PLAYER_HITBOX_HEIGHT;

        const collision = movingEnemies.some((enemy) => {
          const enemyInset = enemy.width * .08;

          const enemyLeft = enemy.x + enemyInset;
          const enemyRight = enemy.x + enemy.width - enemyInset;

          const enemyTop = enemy.y + .9;
          const enemyBottom = enemyTop + ENEMY_HITBOX_HEIGHT;
        ...
        const playerCenterX = playerLeft + PLAYER_HITBOX_WIDTH / 2;

        const insideSite =
          playerCenterX >= SITE_LEFT &&
          playerCenterX <= SITE_RIGHT &&
          playerTop <= SITE_BOTTOM;
                ...
                <div className={`plant-timing-board plant-timing-board--round-${round}`} onClick={commit} style={{backgroundImage:`linear-gradient(rgba(4,8,12,.18),rgba(4,8,12,.18)),url(${carnalitoBackground})`}}>
                  ...
                  {enemies.map((enemy,index) => (
                    <div key={index} className="plant-timing-enemy" style={{left:`${enemy.x}%`,top:`${enemy.y}%`,width:`${enemy.width}%`}}>
                      ...
                      <img className="plant-timing-enemy__sprite" src={sovaDroneSprite} alt="" draggable={false} />
                      <span className="plant-timing-enemy__line" />

                      <div className="plant-timing-enemy__hitbox-debug" />
                    </div>
                  ))}
                  ...
                  <div className={`plant-timing-player plant-timing-player--${phase}`} style={{left:`${playerX}%`,top:`${playerY}%`}}>
                    ...
                    <div className="plant-timing-player__hitbox-debug" />
  ...
  return (
    <div className="career-minigame-intro">
      ...
      <div className="career-minigame-intro__body">
        <div className="career-minigame-intro__character">
          <img src={carnalitoSprite} alt="Carnalito" draggable={false} />
        </div>
        <h3>{language === "es" ? "ENCUENTRA EL MOMENTO PARA PLANTAR" : "FIND THE OPENING TO ENTER"}</h3>

        <p>
          {language === "es"
            ? "Carnalito se moverá horizontalmente esperando el momento correcto. Haz click o presiona SPACE para mandarlo a plantar. Evita los drones enemigos: si chocas con uno, Carnalito será eliminado."
            : "Wingman will move horizontally while waiting for the right moment. Click or press SPACE to send him in to plant the Spike. Avoid the enemy drones: if Wingman collides with one, he will be eliminated."}
        </p>
